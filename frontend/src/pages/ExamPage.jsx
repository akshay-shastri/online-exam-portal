...
function ExamPage() {
    ...
    const submitExam = async () => {
        ...
        questions.forEach((question) => {
            if (answers[question.id] === question.correctAnswer) correctAnswers++;
        });
        ...
        try {
            await API.post("/results", { studentName, examTitle, score: correctAnswers, totalQuestions, percentage });
        } catch (error) {
            console.log("Failed to save result:", error);
        }

        navigate("/result", { state: { score: correctAnswers, totalQuestions } });
    };
        ...
        <div className="min-h-screen bg-[#f0f4ff] dark:bg-gray-950">
            ...
            <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-blue-100 shadow-sm dark:bg-gray-900/90 dark:border-gray-700">
                <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent leading-none">Smart Exam Portal</h1>
                            <p className="text-[10px] text-gray-400 font-medium tracking-widest uppercase mt-0.5">Examination in Progress</p>
                        </div>
                    </div>

                    {/* Timer */}
                    <div className={`flex items-center gap-2.5 px-5 py-2.5 rounded-2xl font-bold text-lg transition-all duration-500 shadow-md ${isCritical ? "bg-red-500 text-white shadow-red-200 animate-pulse" : isWarning ? "bg-orange-400 text-white shadow-orange-200" : "bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-blue-200"}`}>
                        <svg className="w-4 h-4 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className="tabular-nums tracking-wider">{minutes}:{seconds < 10 ? `0${seconds}` : seconds}</span>
                    </div>
            ...
            <div className="max-w-4xl mx-auto px-6 py-10">
                ...
                {!questionsLoading && questionsError && (
                    <div className="text-center py-24 bg-white rounded-2xl border border-dashed border-red-200 shadow-sm dark:bg-gray-800 dark:border-red-700">
                        ...
                        <button onClick={() => navigate("/student-dashboard")} className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 shadow-sm hover:shadow-blue-200 hover:shadow-md">Back to Dashboard</button>
                    </div>
                )}
                ...
                {questions.length > 0 && (
                    <div className="mb-8">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden dark:bg-gray-700">
                            <div className="h-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 transition-all duration-500" style={{ width: `${questions.length > 0 ? (answeredCount / questions.length) * 100 : 0}%` }} />
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-6">
                    ...
                    {questions.map((question, index) => (
                        ...
                        <div key={question.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                            <div className="h-1 w-full bg-gradient-to-r from-blue-400 to-indigo-500" />

                            <div className="p-7">

                                <div className="flex items-start gap-4 mb-6">
                                    <span className="shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-sm font-bold flex items-center justify-center shadow-sm">{index + 1}</span>
                                    <h2 className="text-base font-semibold text-gray-800 leading-relaxed pt-1.5 dark:text-gray-100">{question.questionText}</h2>
                                </div>
                                ...
                                <div className="space-y-3 pl-1">
                                    ...
                                    {[question.optionA, question.optionB, question.optionC, question.optionD].map((option, i) => {
                                        ...
                                        return (
                                            <button key={i} onClick={() => handleOptionSelect(question.id, option)} disabled={submitted} className={`w-full text-left flex items-center gap-4 px-5 py-3.5 rounded-xl border-2 text-sm font-medium transition-all duration-200 ${isSelected ? "border-blue-500 bg-blue-50 text-blue-700 shadow-sm shadow-blue-100" : "border-gray-100 bg-gray-50 text-gray-700 hover:border-blue-300 hover:bg-blue-50/50 hover:text-blue-600"} ${submitted ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}>
                                                <span className={`shrink-0 w-7 h-7 rounded-lg text-xs font-bold flex items-center justify-center transition-all duration-200 ${isSelected ? "bg-blue-500 text-white dark:bg-blue-700 dark:text-blue-100" : "bg-white border border-gray-200 text-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"}`}>{optionLabels[i]}</span>
                                                <span>{option}</span>
                                                {isSelected && (
                ...
                {!submitted && (
                    ...
                    <div className="mt-10 flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-300">{questions.length - answeredCount > 0 ? `${questions.length - answeredCount} question${questions.length - answeredCount !== 1 ? "s" : ""} unanswered` : "All questions answered — ready to submit!"}</p>
                    </div>
                )}
            ...
            <ConfirmModal isOpen={showSubmitConfirm} title="Submit Exam" message={answeredCount < questions.length ? `You have ${questions.length - answeredCount} unanswered question${questions.length - answeredCount !== 1 ? "s" : ""}. Are you sure you want to submit?` : "Are you sure you want to submit your exam? This action cannot be undone."} confirmLabel="Submit" confirmClass="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white" onConfirm={submitExam} onCancel={() => setShowSubmitConfirm(false)} />
